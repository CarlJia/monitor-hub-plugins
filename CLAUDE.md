...
契约 crate 的版本取自 monitor 仓的 tag `monitor-plugin-contract-v<abi>.<n>[.<n>...]`,插件仓按 tag 拉。**这套 tag 规则与校验命令的唯一实现是 `scripts/contract-check.sh`**(ci.yml 与 release.yml 都调它)。只认严格数字点分的正式 tag:带后缀的预发布 tag(`-rc1` 之类)在 `sort -V` 下会排在同版本正式 tag **之后**,而 monitor 的 `publish-contract.yml` 判定失败后**不会**回滚 tag,不过滤就会被三条路径长期一致选中。**tag 指向的提交内容才是被消费的东西**:monitor 的 `publish-contract.yml` 现在会校验「tag 上的 crate 与 main 上的逐字节一致」,在 tag 是过期提交时当场红——2026-09-19 就出过 `v2.0.0` 打在过期提交上、两个插件 release 全部编译失败的事。
...
- **契约 crate 改了、但没在 monitor 打新 tag** —— 三个门都按 `abi_version` 去取现成的 `monitor-plugin-contract-v<abi>.<n>` tag,而 `publish-contract.yml` 只在 tag push 时触发。所以「同 ABI 的契约修复合进 main 却没打 tag」不被任何门发现,插件会对着旧 crate 发版。反向的错(tag 打在过期提交上)已由 monitor 侧的内容一致性校验挡住。
