...
这些码对操作员没有可读性，所以 `plugin.toml` 用 `[[config]]` 声明了两个 kv
...
2. 点该行的「配置」，填 `plugin.toml` 里 `[[config]]` 声明的两项（等价于
...
3. 点「启用」，再用「测试」自检——它会构造一条合成的 `plugin_expiry_soon`
   ...
   > 已安装过旧版的部署：库里的 manifest 是上传时抄的一份，`[[config]]` 要
...
`[[config]]` 里的 key 必须与 `src/lib.rs` 中 `kv_get_string` 读的名字逐字
